                            ...
                            const char	*helper_executable/*=NULL*/)
{
    ...
    LPWSTR ptr = theOne(cmdline);

    BOOL bRet = CreateProcess(NULL,theOne(cmdline),NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
    if (!bRet)
    {	// on failure - try to get the path from the environment
        ...
        if (path)
        {
            ...
            bRet = CreateProcess(NULL,theOne(cmdline),NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
        }
        if (!bRet)
        {
...
wchar_t *CConsoleLogger::theOne(const char* charArray)
{
